...
( function () {
	...
	function initLogViewer() {
		...
		if (
			! wpObject.element ||
			! wpObject.dataviews ||
			! wpObject.dataviews.DataViews
		) {
			return;
		}

		const initialEntries = Array.isArray(
			window.lbsLogViewerConfig.entries
		)
			? window.lbsLogViewerConfig.entries
			: [];
		const fileRows = Array.isArray( window.lbsLogViewerConfig.files )
			? window.lbsLogViewerConfig.files
			: [];
		...
		const __ =
			wpObject.i18n && wpObject.i18n.__
				? wpObject.i18n.__
				: ( text ) => text;
		const { createElement, useState, useMemo, useCallback } =
			wpObject.element;
		...
		const LogViewerApp = () => {
			const [ selectedFile, setSelectedFile ] = useState(
				String( window.lbsLogViewerConfig.selectedFile || '' )
			);
			...
			const loadEntries = useCallback( async ( file ) => {
				...
				try {
					const url = new URL(
						window.lbsLogViewerConfig.restBase + '/entries'
					);
					if ( file ) {
					...
					const response = await window.fetch( url.toString(), {
						headers: {
							'X-WP-Nonce': String(
								window.lbsLogViewerConfig.restNonce || ''
							),
						},
					} );
					...
					setErrorMessage(
						__(
							'Unable to load log entries for the selected file.',
							'load-balanced-sync'
						)
					);
					setEntries( [] );
			...
			return createElement(
				...
				isLoading
					? createElement(
							'p',
							null,
							__( 'Loading entries…', 'load-balanced-sync' )
					  )
		...
		if ( typeof wpObject.element.createRoot === 'function' ) {
			wpObject.element
				.createRoot( mountNode )
				.render( createElement( LogViewerApp ) );
			return;
		}
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		document
			.querySelectorAll( '[data-lbs-copy]' )
